...
bool EpipolarGeometryClass::EstimateRotationAndTranslation(std::vector<cv::Point2f> &pixelPoints0,   // 图像 0 中的特征点像素坐标
    ...
    // 计算归一化平面坐标
    std::vector<Eigen::Vector3d> normPoints0 = GetNormPointsFromPixelPoints(pixelPoints0, CameraMatrix);
    std::vector<Eigen::Vector3d> normPoints1 = GetNormPointsFromPixelPoints(pixelPoints1, CameraMatrix);

    // 调用接口函数
    return EstimateRotationAndTranslation(normPoints0, normPoints1, status, R_c1c0, t_c1c0);
}
...
bool EpipolarGeometryClass::EstimateRotationAndTranslation(std::vector<Eigen::Vector3d> &normPoints0,// 图像 0 中的归一化平面坐标
    ...
    while (maxEpoch > 0) {
        ...
        // 用临时特征点对计算出本质矩阵 E
        double invalidError;
        Eigen::Matrix3d tempE = FindEssentialMatrix(tempPoints0, tempPoints1, invalidError);

        // 根据当前模型 E，计算出所有特征点对的模型误差
        tempErrors = ComputeEpipolarGeometryError(normPoints0, normPoints1, tempE);
        ...
        if (inliersMeanError < minError) {
            ...
            if (inliersPoints0.size() > 4) {
                bestE = FindEssentialMatrix(inliersPoints0, inliersPoints1, invalidError);
            } else {
                bestE = tempE;
            }
        }
    ...
    // 分解本质矩阵 E，获得位姿
    int wrongNums = RecoverPoseFromEssentialMatrix(bestE, inliersPoints0, inliersPoints1, R_c1c0, t_c1c0);
...
bool EpipolarGeometryClass::FindInliersWithEssentialMatrix(std::vector<cv::Point2f> &pixelPoints0,   // 图像 0 中的特征点像素坐标
    ...
    // 调用接口函数
    return FindInliersWithEssentialMatrix(normPoints0, normPoints1, status);
}
...
Eigen::Matrix3d EpipolarGeometryClass::FindEssentialMatrix(std::vector<cv::Point2f> &pixelPoints0,
    ...
    // 调用接口函数
    return FindEssentialMatrix(normPoints0, normPoints1, error);
}
...
Eigen::Matrix3d EpipolarGeometryClass::FindEssentialMatrix(std::vector<Eigen::Vector3d> &normPoints0,
    ...
    std::vector<Eigen::Matrix3d> allE = FindAllEssentialMatriceFivePoints(normPoints0, normPoints1, meanErrors);
...
int EpipolarGeometryClass::RecoverPoseFromEssentialMatrix(Eigen::Matrix3d &E,                     // 输入本质矩阵
    ...
    // 调用接口函数
    return RecoverPoseFromEssentialMatrix(E, normPoints0, normPoints1, R_c1c0, t_c1c0);
}
...
int EpipolarGeometryClass::RecoverPoseFromEssentialMatrix(Eigen::Matrix3d &E,                          // 输入本质矩阵
    ...
    DecomposeEssentialMatrix(E, R0, R1, t0, t1);
    ...
    for (unsigned int i = 0; i < 4; i++) {
        for (unsigned int j = 0; j < normPoints0.size(); j++) {
            ...
            pos = TriangulateOnePoint(R_c0c0, t_c0c0, test[i].second.first, test[i].second.second, normPoints0[j], normPoints1[j]);
            // P_c0
            if (pos[2] > 0) {
...
std::vector<Eigen::Matrix3d> EpipolarGeometryClass::FindAllEssentialMatriceFivePoints(std::vector<cv::Point2f> &pixelPoints0,
    ...
    // 调用接口函数
    return FindAllEssentialMatriceFivePoints(normPoints0, normPoints1, meanErrors);
}
...
std::vector<Eigen::Matrix3d> EpipolarGeometryClass::FindAllEssentialMatriceFivePoints(std::vector<Eigen::Vector3d> &normPoints0,
    ...
    EE = EE.t();
    GetCoeffMatrixFivePoints(EE.ptr<double>(), A.ptr<double>());
    EE = EE.t();
    ...
    meanErrors.clear();
    for (auto &E : allE) {
        std::vector<double> errors = ComputeEpipolarGeometryError(normPoints0, normPoints1, E);
        double error = 0.0;
